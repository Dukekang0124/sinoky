...
(function () {
  ...
  /* ---------- 12. 首访招手邀请：「我先说中文，你回我一句」（S1-M3，P0） ----------
     规划原文（2026-09-13 诺诺IP融合规划 §3.1）：
       现状（实测）主代码零开口分支弹的是一句「关于诺诺的」自我介绍 ——
         它介绍自己 + 派作业，占掉首屏却换不来开口。
       ⚠️ 本层是「抢先覆盖」不是「替换主代码」：主代码那句必须原样保留作兜底
         （招手窗口用尽仍未发出时还原，见下方 waveKick / waveTick），
         所以线上 HTML 里仍会有它 —— 这是预期，不是缺陷。
         故意不在注释里抄原文：否则 grep 分不清「主代码兜底」与「注释残留」。
...
(function (factory) {
  ...
  /* ---------- 14. Day1 通关纪念卡：「学完来华第一天」晒出去（P0 · 传播出口） ----------
